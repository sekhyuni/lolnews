...
export const DivOfDropdownMenuWrapper = styled.div`
    ...
    position: absolute;
    border-radius: 4px;
    background-color: ${({ layoutName }: any) => layoutName === 'main' ? '#fff' : '#000'};
...
export const LinkOfDropdownMenu = styled(Link_)`
    display: block;
    color: ${({ layoutName }: any) => layoutName === 'main' ? '#000' : '#fff'};
    padding: 12px 16px;
    border-radius: 4px;
    text-decoration: none;
    :hover {
        background-color: ${({ layoutName }: any) => layoutName === 'main' ? '#ddd' : '#1e1e1e'};
    }
`;